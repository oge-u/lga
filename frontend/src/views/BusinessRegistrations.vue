      ...
      <table>
        ...
        <tbody>
          <tr v-for="app in applications" :key="app.id">
            ...
            <td>{{ app.contactEmailAddress }}</td>
            <td>{{ app.contactPhoneNumber }}</td>
            <td>{{ formatDate(app.registrationDate) }}</td>
            <td>{{ formatDate(app.applicationDate) }}</td>
            <td>{{ app.status }}</td>
            <td>
              <a
                v-if="app.incorporationDocumentPath"
                :href="app.incorporationDocumentPath"
                target="_blank"
                class="document-link"
              >
                View Document
...
// Fetch business registrations from the backend and transform data
const fetchRegistrations = async () => {
  loading.value = true;
  error.value = '';
  try {
    const adminToken = localStorage.getItem('adminToken');
    const response = await api.get('/admin/applications/business-registrations', {
    ...
    if (response.status === 200) {
      applications.value = (response.data.applications || []).map(app => ({
        ...
        businessSector: app.business_sector,
        businessAddress: app.business_address,
        lgaOfOperation: app.lga_of_operation,
        contactEmailAddress: app.contact_email_address,
        contactPhoneNumber: app.contact_phone_number,
        registrationDate: app.registration_date,
        applicationDate: app.application_date,
        status: app.status,
        incorporationDocumentPath: app.incorporation_document_path,
...
const approveRegistration = async (id) => {
  try {
    ...
    const response = await api.post(
      `/admin/applications/business-registrations/${id}/approve`,
      {},
      { headers: { Authorization: `Bearer ${adminToken}` } }
    );
    if (response.status === 200) {
...
const rejectRegistration = async (id) => {
  try {
    const adminToken = localStorage.getItem('adminToken');
    const response = await api.post(
      `/admin/applications/business-registrations/${id}/reject`,
      {},
      { headers: { Authorization: `Bearer ${adminToken}` } }
    );
    if (response.status === 200) {
